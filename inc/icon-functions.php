...
function seasaltpress_include_svg_icons() {
	...
	$svg_icons = get_parent_theme_file_path( '/assets/icons/symbol-defs.svg' ); //seasaltpress changed to new svg set.
 ...
 */
function seasaltpress_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	// Get supported social icons.
	$social_icons = seasaltpress_social_links_icons();

	// Change SVG icon inside social links menu if there is supported URL.
	if ( 'social' === $args->theme_location ) {
		foreach ( $social_icons as $attr => $value ) {
			if ( false !== strpos( $item_output, $attr ) ) {
				$item_output = str_replace( $args->link_after, '</span>' . seasaltpress_get_svg( array( 'icon' => esc_attr( $value ) ) ), $item_output );
			}
		}
	}
...
add_filter( 'walker_nav_menu_start_el', 'seasaltpress_nav_menu_social_icons', 10, 4 );
 ...
 */
function seasaltpress_social_links_icons() {
	...
	/**
	 * Filter Sea Salt Press social links icons.
	 *
	 * @since Sea Salt Press 1.0
	 *
	 * @param array $social_links_icons
	 */
	return apply_filters( 'seasaltpress_social_links_icons', $social_links_icons );
}
